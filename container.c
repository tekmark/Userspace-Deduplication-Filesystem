...
container_t* container_init(){
  ...
  container->header = (container_header_t*)container->blk[0];
...
uint32_t container_write( container_t *container, uint32_t new_id) {
  ...
  uint32_t offset = container_id * CONTAINER_SISE + BLK_SIZE;  
  uint32_t i = 0; 
...
uint32_t *container_read(container_t *container, uint32_t container_id) {
  ...
  pread(li->fd, container->blks[0], CONTAINER_SIZE, offset);
  container->offset = CONTAINTER_SIZE;  //container is full
  return 0; 
}

void container_clean( container_t *container, uint32_t *live_seg, 
                      uint32_t size) {
  
}

int container_add_seg( container_t *container, char *seg_buf) {
...
int container_get_seg( container_t *container, uint32_t seg_offset, 
                       char *seg_buf) {
