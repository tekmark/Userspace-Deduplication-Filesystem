...
int c_header_get_seg_info(container_header_t *header, int seg_no, seg_tbl_r_t *r) {
    ...
    r->seg_no = seg_no;
    //copy fingerprint
    memcpy(r->fp.fingerprint, header + entry_offset + SEG_TBL_ENTRY_FP_OFFSET,
        FINGERPRINT_LEN);
    int blk_offset = *(uint32_t*)(header + entry_offset + SEG_TBL_ENTRY_BLK_OFFSET_OFFSET);
...
int c_header_add_seg_info(container_header_t *header, seg_tbl_r_t *r){
    ...
    assert(header->id);
    memcpy(header + seg_offset + SEG_TBL_ENTRY_SEG_NO_OFFSET, &seg_tbl_size,
            sizeof(uint32_t));
    assert(header->id);
    memcpy(header + seg_offset + SEG_TBL_ENTRY_FP_OFFSET, r->fp.fingerprint,
            FINGERPRINT_LEN);
    memcpy(header + seg_offset + SEG_TBL_ENTRY_BLK_OFFSET_OFFSET, &r->blk_offset,
            sizeof(r->blk_offset));
    assert(header->id);
    //update seg_tbl_size;
...
int c_header_find_seg_by_fp(container_header_t *header, fp_t *fp, seg_tbl_r_t *r) {
    ...
    for (i = 0; i < tbl_size; ++i) {
        ...
        if (!fp_cmp(fp, &r->fp)) {      //if found.
            r->seg_no = *(uint32_t*)(header + entry_offset);
            r->blk_offset = *(uint32_t*)(header + entry_offset + SEG_TBL_ENTRY_BLK_OFFSET_OFFSET);
            r->seg_size = *(uint32_t*)(header + entry_offset + SEG_TBL_ENTRY_SEG_SIZE_OFFSET);
            return r->seg_no;
        }
    }
    return -1;
